...
// =================多=====================
    ...
    PtrStep<unsigned char> nextMask;
  ...
  // 反向投影搜索匹配点===============================================  
    ...
    search (int & x, int & y, float3& n, float3& d, float3& s) const
    {
        ...
        if(ukr.x < 0 || ukr.y < 0 || ukr.x >= cols || ukr.y >= rows || vcurr_cp.z < 0){
            ...
            // if(outErrorSurface) surf2Dwrite(0.0f, outErrorSurface, x*sizeof(float), y); //FIXME uncomment
            return false;
        }
...
// 3D点对应的 目标类别mask误差 
        ...
        if(nextMask.ptr(y)[x] != maskID){
            if(outErrorSurface) surf2Dwrite(1.0f, outErrorSurface, x*sizeof(float), y); //FIXME remove
            return false;
        } else {
