 ...
 * SLIC超像素分割算法  SLIC（simple linear iterativeclustering），即简单的线性迭代聚类 
 它是2010年提出的一种思想简单、实现方便的算法，将彩色图像转化为CIELAB颜色空间和XY坐标下的5维特征向量，
 然后对5维特征向量构造距离度量标准，对图像像素进行局部聚类的过程。
...
class Slic {
 public:
  Slic() {}
  Slic(unsigned width, unsigned height, 
       int spixelSize,
       /*float scale,*/
       gSLICr::COLOR_SPACE colorSpace = gSLICr::XYZ);
