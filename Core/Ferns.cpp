                     ...
                     int srcTime, const float threshold) 
{
  ...
  Frame* frame = new Frame(num, frames.size(), pose, 
                           srcTime, width * height, 
...
Eigen::Matrix4f Ferns::findFrame(std::vector<SurfaceConstraint>& constraints,
                                 ...
                                 const int time, const bool lost) 
{
  lastClosest = -1;
  ...
  if (minId != -1 && blockHDAware(frame, frames.at(minId)) > 0.3) 
  {
    ...
    assert(false); // This function is not implemented anymore!! FIXME
